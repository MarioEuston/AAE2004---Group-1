...
while True:
    sx = np.random.randint(-9,60)  # [m]
    sy = np.random.randint(-9,60)  # [m]
    gx = np.random.randint(-9,60)  # [m]
    gy = np.random.randint(-9,60)  # [m]
    if math.sqrt((sx-gx)**2+(sy-gy)**2)>50:
        break


class AStarPlanner:

    def __init__(self, ox, oy, resolution, rr, tc_x, tc_y):
        """
